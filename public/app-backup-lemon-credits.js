...
upgradeBtn.addEventListener("click", async () => {
  const confirmed = confirm(
    "This will upgrade your account to Pro in test mode. Continue?"
  );
  ...
  try {
    const data = await apiRequest("/api/account/upgrade", {
      method: "PATCH",
    });
...
async function loadDashboardStats() {
  try {
    ...
    if (usage.plan === "trial") {
      remainingStat.textContent = usage.scansRemaining;
    } else {
...
async function loadUsage() {
  try {
    ...
    if (usage.plan === "trial") {
      ...
      const trialDate = new Date(usage.trialEndsAt).toLocaleDateString();

      usageInfo.textContent = `Trial: ${usage.scansUsed}/${usage.scanLimit} scans used | Remaining: ${usage.scansRemaining} | Ends: ${trialDate}`;
      ...
      if (!usage.canScan) {
        ...
        upgradeBtn.classList.add("hidden");
      usageInfo.textContent = `Plan: ${usage.plan} | Unlimited scans`;
      analyzeBtn.disabled = false;
      analyzeBtn.textContent = "Analyze Message";
      messageInput.disabled = false;
